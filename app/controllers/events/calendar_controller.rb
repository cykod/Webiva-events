...
class Events::CalendarController < ParagraphController
  ...
  editor_for :upcoming_list, :name => 'Targeted Upcoming Events', :features => [:events_calendar_upcoming_list ], :inputs => [[:target,'Event Target',:target ]]
  ...
  class MonthListOptions < HashModel
    attributes :detail_page_id => nil, :overlay => true, :block_width => 80, :block_height => 90
  ...
  class UpcomingListOptions < HashModel
    attributes :max_events => 10, :overlay => true, :detail_page_id => nil
  ...
  class EventDetailOptions < HashModel
    attributes :message_page_id => nil, :text_page_id => nil, :personal_events => true
    
    boolean_options :personal_events
